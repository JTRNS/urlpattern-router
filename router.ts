...
type Handler<T = any, U = any> = (req: Request, ctx: T, ...args: U[]) => Response | Promise<Response>
type RequestHandler<P, T> = Handler<T & { params: RoutePath<P> }>
type MiddlewareHandler<P, T> = Handler<T & { params: RoutePath<P>, next(): Response | Promise<Response> }>
...
export class Router<T extends object = Record<string | number | symbol, never>, Env extends any = any> {
  ...
  add<P extends string>(method: string | string[], path: P, handler: RequestHandler<P, T>): void {
  ...
  use<P extends string>(path: P, handler: MiddlewareHandler<P, T>) {
    ...
    this.#middlewares.push([new URLPattern({ pathname: path }), [handler as MiddlewareHandler<string, T>]]);
  }
  ...
  #handleRoute(request: Request, route: Route, context: T, ...args: Env[]): Promise<Response> {
    const handlers = route.handlers;
    let index = handlers.length;
    const params = 'params' in route ? route.params : {}
    const next = async (response: Response = new Response()): Promise<Response> => {
      index--;

      if (index > 0) {
        response = await handlers[index](request, Object.assign(context, { next }, params), ...args);
      } else {
        // remove next method from context?
        response = await handlers[index](request, Object.assign(context, { params }), ...args);
      }
      return response;
    };
    return next();
  ...
  #attachMiddleware(route: Route, middlewares: MiddlewareHandler<string, T>[][]) {
  ...
  match(method: string, path: string): Handler<T, Env> {
    ...
    if (cached) {
      return (req: Request, ctx: T, ...args: Env[]) => this.#handleRoute(req, cached, ctx, ...args);
    }
    ...
    const staticRoute: StaticRoute | undefined = this.#staticRoutes?.[method]?.[path];
    if (staticRoute) {
      console.count('static route')
      this.#attachMiddleware(staticRoute, collection);
      this.#cachedRoutes[method] = { [path]: staticRoute };
      return (req: Request, ctx: T, ...args) => this.#handleRoute(req, staticRoute, ctx, ...args);
    }

    const routes = this.#dynamicRoutes[method];
    if (!routes) {
      // TODO: Add configurable Method Not Allowed Handler
      return (req: Request, ctx: T) => new Response('Method Not Allowed', { status: 405 });
    }
    for (let i = routes.length; i--;) {
      ...
      if (params !== null) {
        ...
        return (req: Request, ctx: T, ...args) => this.#handleRoute(req, r, ctx, ...args);
      }
    }
    // TODO: Add configurable Not Found Handler
    return (req: Request, ctx: T) => new Response('Not Found', { status: 404 });
  }
